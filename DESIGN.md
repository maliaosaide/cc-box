...
- [x] 项目初始化（Go module + cobra CLI 框架）
- [x] config 模块（config.toml 读写 + 系统密钥环）
- [x] WebDAV 客户端（PROPFIND/GET/PUT/MKCOL/DELETE，含 ETag 解析）
- [x] 跨平台规范化器（路径/换行/大小写规范化）
- [x] 文件扫描器（扫描 ~/.claude/，应用排除规则 + 规范化哈希）
- [x] 快照管理（创建快照、链式存储、祖先查找）
- [x] Object 存储（文件内容加密 → 上传/下载到 WebDAV）
- [x] init 命令（bubbletea 交互式向导 + 首次快照创建）
- [x] push 命令（扫描变更 → 加密 → 上传 → 乐观锁更新 HEAD）
- [x] pull 命令（下载远程快照 → 差异对比 → 下载 objects → 应用）
- [x] status 命令（本地 vs 远程差异概览）
...
- [x] 配置文件页面：文件树组件（展开/折叠 + 状态图标）
- [x] 配置文件页面：文件内容预览面板
- [x] 配置文件页面：diff 查看器（本地 vs 云端对比）
- [x] 配置文件页面：筛选器（全部/已变更/冲突）
- [x] 配置文件页面：冲突三选一面板（本地/远程/合并预览）
- [x] 配置文件页面：排除文件操作（添加到排除规则）
- [x] 配置文件页面：批量操作（推送所有/拉取所有）
- [x] diff 命令（CLI 侧）
...
- [x] 二进制页面：Tab 切换（claude/uv/uvx/uvw）
- [x] 二进制页面：版本列表（当前使用/仅云端/本地+云端状态）
- [x] 二进制页面：上传/下载/切换操作 + 进度条 + 取消
- [ ] 二进制页面：版本操作菜单（动态显示可用操作）
- [x] 二进制页面：存储空间统计 + 清理旧版本
- [x] 项目页面：已追踪项目列表（本地路径、MCP servers 数量）
- [x] 项目页面：项目详情展开（.claude.json 内容预览 + 变更对比）
- [x] 项目页面：orphan 管理（关联本地目录 / 删除云端配置）
- [x] 项目页面：添加项目路径 + 同步所有项目

### Phase 3d: 历史、设置与系统托盘

目标：历史记录、设置页面、系统托盘、自动同步。

- [x] 历史页面：快照时间线列表（设备/摘要/二进制版本）
- [x] 历史页面：快照详情展开（文件变更列表 + 行数统计）
- [x] 历史页面：筛选器（设备/时间范围）+ 加载更多
- [x] 历史页面：查看完整 diff + 回滚操作
- [x] 设置页面：连接 tab（WebDAV 配置 + 测试连接）
- [x] 设置页面：加密 tab（状态显示 + rekey）
- [x] 设置页面：同步 tab（快照保留/冲突策略/自动上传）
- [x] 设置页面：路径 tab（三个路径配置 + 浏览按钮）
- [x] 设置页面：排除规则 tab（默认/自定义规则 + 添加/删除）
- [x] 设置页面：设备 tab（设备列表 + 重命名/移除）
